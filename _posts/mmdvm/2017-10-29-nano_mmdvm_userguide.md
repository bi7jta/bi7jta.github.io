...
3）DMR模式手台如何进入46001？调到对应的频道，按下PTT
4）如何查看别人的DMR-ID？https://www.dmr-marc.net/cgi-bin/trbo-database/  
）该用哪一种模式？我刷机时为了让大家快速入门，配置了两种模式（YSFC4FM/DMR），建议上手后只保留一种模式，减少频繁自动切换模式的延时，如图。   
